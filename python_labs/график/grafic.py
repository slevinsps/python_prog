...
иями! ')
        ...
        v = 2048 * n ** 12 - 6144 * n ** 10 + 6912 * n ** 8 - 3584\
            * n ** 6 + 840 * n ** 4 - 72 * n ** 2 + 1
        max = v
        min = v
        mint = n
        l = n
        l1 = round(l, 5)
        k1 = round(k, 5)
        while l1 < k1:  # Находим максимальное и минималное значение
            l1 = round(l, 5)
            k1 = round(k, 5)
            v = 2048 * l ** 12 - 6144 * l ** 10 + 6912 * l ** 8 - \
                3584 * l ** 6 + 840 * l ** 4 - 72 * l ** 2 + 1
            if v > max:
                max = v
            if v < min:
        ...
        print('Минимальное значение w = ',end='')
        if abs(min) >= 10000:
            print('{:.3e}'.format(min),end = '')
        else:
            print('{:.3f}'.format(min),end = '')
        print(' оно достигается при t = {:.3f}'.format(mint))
        print('\n')
        print('График функции: w = 2048*t^12-6144*t^10+6912*t^8-\
3584*t^6+840*t^4-72*t^2+1')
        ...
        while True:  # Печатаем график
            ...
            k1 = round(k, 5)
            if l1 <= k1:
                w = 2048 * l ** 12 - 6144 * l ** 10 + 6912 * l ** \
                    8 - 3584 * l ** 6 + 840 * l ** 4 - 72 * l ** 2 + 1
