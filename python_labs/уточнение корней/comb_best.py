...
def find_roots(f, a=-10, b=10, h=2, maxi=1000, eps=1e-7):
    ...
    while True:
        ...
        if abs(f(nach)) < 1e-5:
            ...
            it = 0
            err = 0
            print("|{:3d}|{:7.2f}|{:7.2f}|{:9.4f}\t|{:7.3e}\t|{:3d}\t |{:3d}\t|".format(k, nach, kon, nach, f(nach), it,
                                                                                        err))
            print("-----------------------------------------------------------------")
            korni.append(nach)
        else:
            ...
            if f(nach) * f(kon) < 0:
                ...
                if r == None:
                    ...
                    err = 2
                    print(
                        '|{:3d}|{:7.2f}|{:7.2f}|  ---   \t|   ---   \t|{:3d}\t |{:3d}\t|'.format(k, nach, kon, it, err))
                    print("-----------------------------------------------------------------")
                else:
                    err = 0
                    print(
                        "|{:3d}|{:7.2f}|{:7.2f}|{:9.4f}\t|{:7.3e}\t|{:3d}\t |{:3d}\t|".format(k, nach, kon, r, f(r), it,
                                                                                              err))
                    print("-----------------------------------------------------------------")
        ...
        if nach == b:
            if f(nach) == 0:
                ...
                err = 0
                print(
                    "|{:3d}|{:7.2f}|{:7.2f}|{:9.4f}\t|{:7.3e}\t|{:3d}\t |{:3d}\t|".format(k, nach, kon, nach, f(nach),
                                                                                          it,
                                                                                          err))
                print("-----------------------------------------------------------------")
                korni.append(kon)
...
def comb(f, a, b, maxi, eps):
    if f1(a) != 0 and f1(b) != 0:
        if not a < a - f(a) / f1(a) < b and not a < b - f(b) / f1(b) < b:
            return None, None
    if f1(a) == 0 and f1(b) == 0:
        return None, None
    elif f1(a) != 0:
