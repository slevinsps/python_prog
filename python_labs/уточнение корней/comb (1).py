...
def find_roots(f, a=0, b=10, h=3, maxi=1000, eps=1e-7):
    t3 = 0
    print("-----------------------------------------------------------------")
...
def comb(f, a, b, maxi, eps):
    ...
    while abs(a - b) > eps:
        ...
        try:
            if f(a) * f2(a) < 0:
                if not a1 <= a - f(a)*(b-a)/(f(b)-f(a)) <= b1:
                    a = get_x(f,a,b)
            elif f(a) * f2(a) > 0:
                if not a1 <= a-f(a)/f1(a) <= b1:
                    a = get_x1(f,a)
            if f(b) * f2(b) < 0:
                if not a1 <= b - f(b)*(a-b)/(f(a)-f(b)) <= b1:
                    b = get_x(f,b,a)
            elif f(b) * f2(b) > 0:
                if not a1 <= b - f(b)/f1(b) <= b1:
                    b = get_x1(f,b)
        except ZeroDivisionError:
            return None, None
        k += 1
